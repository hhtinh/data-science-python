...
df = pd.read_csv('Regular_to_Business.csv', index_col=0)

df_info(df)


df['UPGRADE'] = np.where((df['UPGRATE_AFTER_1MONTHS'] == 'YES') | \
(df['UPGRATE_AFTER_2MONTHS'] == 'YES') | \
(df['UPGRATE_AFTER_3MONTHS'] == 'YES'), 1, 0)

df['UPGRADE'].value_counts()

df['CUSTOMER_ID'].value_counts().head(10)

df['CUSTOMER_ID'].groupby(df['LIST_MONTH']).count()


df_May = df[df['LIST_MONTH'] == '2018-05-31']

df_May['UPGRADE'].value_counts()


df_Mar = df[df['LIST_MONTH'] == '2018-03-31']

df_Mar['UPGRADE'].value_counts()

df_Mar['V_LIST'].value_counts()

pd.crosstab(df_Mar['UPGRADE'], df_Mar['V_LIST'])

df_Mar['OWNRENTDESC'].value_counts()

df_Mar['BRANCH_NAME'].value_counts()


#df_Mar.to_csv("Reg2Biz_Mar.csv", index=False)
df_Mar.to_pickle("Reg2Biz_Mar.pkl.gz")
